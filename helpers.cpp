...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	for(int row=0; row<grid.size(); row++) {
		vector<float> newRow;
		for(int cell=0; cell<grid[row].size(); cell++) {
			newRow.push_back(grid[row][cell] / total);
		}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	window.push_back({corner_prob, adjacent_prob, corner_prob});
	window.push_back({adjacent_prob, center_prob, adjacent_prob});
	window.push_back({corner_prob, adjacent_prob, corner_prob});
...
//int main() {
//	vector < vector < char > > map1 = read_map("maps/m1.txt");
// 	show_grid(map1);
//	cout << endl;
//
//	vector < vector < char > > map2 = read_map("maps/m2.txt");
//	show_grid(map2);
//	cout << endl;
//
//	vector < vector < char > > half = read_map("maps/half_red.txt");
//	show_grid(half);
//	cout << endl;
//
//	show_grid(zeros(3, 3));
//	cout << endl;
//
//	vector< vector<float> > newGrid(3, {3.0, 2.0, 5.0});
//	show_grid(newGrid);
//	cout << endl;
//
//	show_grid(normalize(newGrid));
//	cout << endl;
//
//	show_grid(blur(newGrid, 5.0));
//	cout << endl;
//
// 	return 0;
//}
